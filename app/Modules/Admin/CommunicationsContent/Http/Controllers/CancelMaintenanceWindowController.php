...
final class CancelMaintenanceWindowController
{
    public function __invoke(Request $request, string $maintenanceId, AdminAuditLogger $audit): JsonResponse
    {
        ...
        } $m->forceFill(['status' => 'cancelled', 'cancelled_at' => now()])->save();
        $audit->write('maintenance.window.cancelled', $request->user(), $request->attributes->get('admin_session'), 'maintenance_window', $m->id, reason: (string) $request->input('reason', 'Maintenance cancelled'), request: $request);
