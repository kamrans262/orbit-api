...
final class RevokeAdminUserDeviceController
{
    public function __invoke(AdminReasonRequest $request, int $userId, string $deviceId, AdminDeviceOperationsService $service): JsonResponse
    {
        ...
        $device = $service->revoke($user, $device, AdminOperationContext::admin($request), AdminOperationContext::session($request), (string) $request->validated('reason'), $request);

        return AdminApiResponse::success($request, $service->present($device), message: 'Device revoked.');
    }
}
