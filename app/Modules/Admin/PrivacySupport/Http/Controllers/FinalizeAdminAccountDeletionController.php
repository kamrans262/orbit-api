...
final class FinalizeAdminAccountDeletionController
{
    public function __invoke(Request $request, string $deletionId, AdminDeletionService $service, PrivacyPresenter $presenter): JsonResponse
    {
        ...
        try {
            $result = $service->finalize($deletion, $admin, $session, $data['reason'], $request);
        ...
        return AdminApiResponse::success($request, ['result' => $result, 'deletion' => $presenter->deletion($deletion->refresh())]);
    }
}
