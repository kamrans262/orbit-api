...
foreach ($path in @($apiRoutes, $consoleRoutes, $providersFile)) {
    ...
    $backup = "$path.pre-admin-privacy-support-m5-backup"
...
Add-OrbitRequireLine -Path $apiRoutes -Line "require __DIR__.'/privacy_support.php';"
Add-OrbitRequireLine -Path $apiRoutes -Line "require __DIR__.'/admin_privacy_support.php';"
Add-OrbitRequireLine -Path $consoleRoutes -Line "require __DIR__.'/console_privacy_support.php';"

$providerClass = 'App\Providers\PrivacySupportServiceProvider::class'
...
Write-Host ''
Write-Host 'Orbit Admin Privacy / Compliance / Support Milestone 5 wiring installed.'
Write-Host 'Next run one command at a time:'
Write-Host '  php artisan migrate'
Write-Host '  php artisan orbit:admin:sync-rbac'
Write-Host '  php artisan orbit:privacy:sync-identity-requests'
Write-Host '  php artisan optimize:clear'
Write-Host '  vendor\bin\pint'
Write-Host '  vendor\bin\pint --test'
Write-Host '  php artisan test tests\Feature\Api\Admin\V1\AdminPrivacyComplianceSupportTest.php'
...
Write-Host '  php artisan route:list --path=admin/v1/privacy'
Write-Host '  php artisan route:list --path=admin/v1/support'
Write-Host '  php artisan route:list --path=v1/privacy'
Write-Host '  php artisan route:list --path=v1/support'
