...
it('renders every canonical M1 M2 and M3 administration view through one shell without embedding operational data', function (): void {
    $this->get('/admin')
        ->assertOk()
    ...
    $this->get('/admin/operations/sos')
        ->assertOk()
        ->assertSee('data-orbit-canonical-shell="v1"', false)
        ->assertSee('data-orbit-view="sos-index"', false)
        ->assertDontSee('data-admin-auth-dialog', false);

    $this->get('/admin/operations/sos/example-sos')
        ->assertOk()
        ->assertSee('data-orbit-canonical-shell="v1"', false)
...
it('uses real canonical navigation for users circles and SOS without parallel UI shells', function (): void {
    ...
    $response
        ...
        ->assertSee('href="'.route('admin.console.operations.sos.index').'"', false)
        ->assertDontSee('Users UI is next')
        ->assertDontSee('Safety / SOS</span><small>Next UI milestone', false)
...
it('registers one canonical route family for the consolidated admin console', function (): void {
    expect(Route::has('admin.console.dashboard'))->toBeTrue()
        ...
        ->and(Route::has('admin.console.operations.sos.index'))->toBeTrue()
        ->and(Route::has('admin.console.operations.sos.show'))->toBeTrue();
});

it('keeps the canonical assets and removes obsolete separate admin UI architectures', function (): void {
    expect(is_file(base_path('resources/css/admin-console.css')))->toBeTrue()
        ...
        ->and(is_file(base_path('resources/js/admin-console/sos.js')))->toBeTrue()
        ->and(is_file(base_path('resources/js/admin-console/sos-contract.js')))->toBeTrue()
